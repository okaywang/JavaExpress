...
public class ThriftMapTemplate extends AbstractCollectionCodeTemplate {
	...
	public String genEncode(ThriftServiceFunc func, String space, ThriftService service) {
		...
		buf.append(formatWriteFieldBegin("success", Constants._TTYPE_MAP, "0", space + "\t"));
	...
	public String genEncode(ThriftServiceFuncParam param, String space, ThriftService service) {
		...
		buf.append(formatWriteFieldBegin(paramName, Constants._TTYPE_MAP, param.getParamIndex(), space + "\t"));
		buf.append(genericEncoderCode(param.getParamType(), space + "\t", "argMap", 0, service));
		buf.append(formatWriteFieldEnd(space + "\t"));
		buf.append(formatIfEnd(space));
		return buf.toString();
	}
	...
	public String genDecode(ThriftServiceFuncParam param, String space, ThriftService service) {
		StringBuffer buf = new StringBuffer();
		buf.append(formatIfEqual(space, new String[] { "field.type", Constants._TTYPE_MAP }));
		buf.append(genericDecodeCode(param.getParamType(), space + "\t", "map0", 0, service));
		buf.append(space + "\t" + "args." + param.getParamName() + " = map0;").append("\r\n");
	...
	public String genEncode(ThriftStructField field, String space, ThriftStruct struct) {
		...
		buf.append(
				formatWriteFieldBegin(field.getFieldName(), Constants._TTYPE_MAP, field.getFieldIndex(), space + "\t"));
	...
	public String genDecode(ThriftStructField field, String space, ThriftStruct struct) {
		StringBuffer buf = new StringBuffer();
		buf.append(formatIfEqual(space, new String[] { "field.type", Constants._TTYPE_MAP }));
		buf.append(genericDecodeCode(field.getFieldType(), space + "\t", "map0", 0, struct));
		String func = getFuncName("set", field.getFieldName());
		buf.append(space + "\t" + "obj." + func + "(map0);").append("\r\n");
		buf.append(formatElse(space));
		buf.append(formatSkip(space + "\t"));
		buf.append(space + "}").append("\r\n");
		return buf.toString();
